...
| — | `INPUT-manifest.md` | Docs | Populated |
| — | `CACHE-manifest.md` | Docs | Populated (2026-05-20) |
| — | `pipeline.md` | Docs | This file |
...
Extracts variable-level and value-level labels, then writes four CSVs.
...
Full-transport of both `.sav` files to a SQLite staging database.
...
- SQLite database — tables `cchs_2010` and `cchs_2014` (raw, all variables)
- Parquet backups — `cchs_2010.parquet`, `cchs_2014.parquet`
...
- SQLite database — tables `cchs_analytic` and `sample_flow`
- Parquet files — `cchs_analytic.parquet`, `sample_flow.parquet`
...
- Confirm both tables are present in the staging database.
- Row counts: ~62,909 (2010) and ~63,522 (2014).

**After Lane 2**:

- Check the `sample_flow` table against the reference in CACHE-manifest.
...
See INPUT-manifest § Cross-Variable Dependencies for the full rationale.
